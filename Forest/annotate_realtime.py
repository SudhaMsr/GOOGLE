...
import cv2
from deep_sort_realtime.deepsort_tracker import DeepSort  # https://github.com/levan92/deep_sort_realtime?tab=MIT-1-ov-file
...
if TRACKER_NAME == "deepSORT":
    ...
    tracker = StrongSORT(model_weights=Path("Pretrained_networks/Track/osnet_x0_25_market1501.pt"), device='cpu', fp16=False)
...
def realTimeAnnotate(frame):
    ...
    angles = getAngles(boxes, frame)

    distances = distancesEstimation(boxes, angles)
...
def objectTracking(boxes, frame):
    ...
    if TRACKER_NAME == "deepSORT":
        ...
        for trnp in tracks:
            ...
            cv2.putText(frame, f"{model.names[tr[5]]}{str(tr[4])}", (tr[0], tr[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 225), 2)
...
def objectsDetection(frame):
    ...
    # draw boxes
    # drawBoxes(frame, boxes, model.names)

    return boxes


def distancesEstimation(boxes, angles):
    return 0
...
def getAngles(boxes, frame):
    vehicles = []

    for box in boxes:
        xyxy = box.xyxy[0]
        x1, y1, x2, y2, name = int(xyxy[0]), int(xyxy[1]), int(xyxy[2]), int(xyxy[3]), model.names[int(box.cls)]
        if name in ['car', 'truck', 'bus']:
            vehicles.append(np.array(cv2.cvtColor(cv2.resize(frame[y1:y2, x1:x2], (128, 128)), cv2.COLOR_BGR2RGB)) / 255)
    ...
    for box in boxes:
        ...
        x1, y1, x2, y2, name = int(xyxy[0]), int(xyxy[1]), int(xyxy[2]), int(xyxy[3]), model.names[int(box.cls)]
        if name in ['car', 'truck', 'bus']:
            angles.append(predicted[cnt][0] * 360)
